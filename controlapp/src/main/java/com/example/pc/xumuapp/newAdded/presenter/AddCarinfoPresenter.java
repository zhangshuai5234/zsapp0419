...
public class AddCarinfoPresenter extends BasePresenter<AddCarinfoView> {
    public AddCarinfoPresenter(AddCarinfoView view) {
        attachView(view);
        apiStores = ApiClient.retrofit(HostType.INTERNAL).create(ApiService.class);
    }

    public void savecarinfoData(Map<String,String> carinfomap) {
        String login = "true";
        String ajax = "json";
        addSubscription(apiStores.addcarinfo(carinfomap), new Subscriber<AddcarinfoModel>() {
            @Override
